export class Color {
    ...
    toString(type, fixation) {
        switch (type) {
            ...
            case "hex8":
                return "#" + this.toHexNumber() + this.a.toString(16).padStart("0", 2);
    ...
    blend(color=new Color(), blendmode="normal") {
        ...
        const channelBlend = blendingFunction => {
    ...
    /**
     * Creates a linearly fading gradient between two colors, based on their red, green, and blue values, and returns a color based on a selected point.
     * @param {Color} [color=new Color()] The second color with which the gradient should be formed.
     * @param {number} [x=.5] With `this` at 0 and `color` at 1, a float representing the progression through the gradient.
     * @return {Color}
     */
